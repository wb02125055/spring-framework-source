...
public class CglibSubclassingInstantiationStrategy extends SimpleInstantiationStrategy {
	...
	private static class CglibSubclassCreator {
		...
		public Object instantiate(@Nullable Constructor<?> ctor, Object... args) {
			// 使用cglib来生成bean实例对象的子类class。Java中生成代理对象，通常情况下有两种，一种是JDK的动态代理（但是需要实现接口）
			// 还有一种就是Cglib动态代理，不需要实现接口即可。此处使用cglib先生成bean实例的子类，然后通过创建子类的对象触发父类对象的创建.
			...
			if (ctor == null) {
				...
				try {
					// 使用cglib生成的子类class来获取子类的构造函数
					Constructor<?> enhancedSubclassConstructor = subclass.getConstructor(ctor.getParameterTypes());
					// 生成子类对应的实例。这个时候会通过super调用父类的构造函数进行实例的创建.
