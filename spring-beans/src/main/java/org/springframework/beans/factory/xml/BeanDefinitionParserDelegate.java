...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		 ...
		 *    @Override
		 *    public void init() {
		 *        registerBeanDefinitionParser("my-tags", new MyTagParser(TagBean.class, true));
		 *    }
		 * }
		 * public class MyTagParser implements BeanDefinitionParser {
		...
		/**
		 * 该步骤中会调用MyNameSpaceHandler的init方法。
		 *   加载自定义的Parser。默认实现类为DefaultNameSpaceHandlerResolver
		 *   并将所有的Parser放入到一个map集合中：Map<String, BeanDefinitionParser> parsers = new HashMap<>();
		 *   在下面执行parse方法的时候，会根据bean的名称，从parsers中找到对应的BeanDefinitionParser的实例，并调用其parse方法.
		 */
		NamespaceHandler handler = this.readerContext.getNamespaceHandlerResolver().resolve(namespaceUri);
		// 如果未解析到命名空间uri对应的命名空间解析类，直接返回.
		if (handler == null) {
			error("Unable to locate Spring NamespaceHandler for XML schema namespace [" + namespaceUri + "]", ele);
			return null;
		}
		// 调用NamespaceHandler的parse方法，自定义标签时需要实现NamespaceHandlerSupport接口.
		//  NamespaceHandlerSupport接口的父接口为NamespaceHandler

		// 此处的handler已经在上一步的resolve方法中实例化成了对应的实现类对象
